...
function getMissingBlocks(callback) {
  db.manyOrNone("select series.num from (select generate_series(3600000, 4000000) as num) series left join blocks bl on series.num = bl.number where bl.number is null;", [])
    .then(callback)
    .catch((err) => {
      console.log("****************************************** ERROR ******************************************")
      console.log(err)
